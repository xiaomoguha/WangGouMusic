...
static void generateCrashReport(int signalNum, const char *signalName
#ifdef Q_OS_WIN
                                ...
                                )
{
    ...
    // 时间戳
    writeFmt(fd, "崩溃时间: %04d-%02d-%02d %02d:%02d:%02d\n",
             lt->tm_year + 1900, lt->tm_mon + 1, lt->tm_mday,
             lt->tm_hour, lt->tm_min, lt->tm_sec);
    ...
    if (ep) {
        ...
        while (StackWalk64(machineType, hProcess, GetCurrentThread(),
            ...
            if (SymFromAddr(hProcess, addr, &disp, symbol)) {
                ...
                writeFmt(fd, "  [%2d] %s!%s +0x%llx  (0x%llx)\n",
                         frameNum, modName, symbol->Name,
    ...
    if (ep) {
        char dumpFile[1100];
        snprintf(dumpFile, sizeof(dumpFile), "%s/crash_%04d%02d%02d_%02d%02d%02d.dmp",
                 s_logDir,
                 lt->tm_year + 1900, lt->tm_mon + 1, lt->tm_mday,
                 lt->tm_hour, lt->tm_min, lt->tm_sec);
        HANDLE hFile = CreateFileA(dumpFile, GENERIC_WRITE, 0, nullptr,
...
static LONG WINAPI windowsExceptionFilter(EXCEPTION_POINTERS *ep)
{
    ...
    switch (code) {
        case EXCEPTION_ACCESS_VIOLATION:         excName = "ACCESS_VIOLATION (内存访问违规)"; break;
        ...
        case EXCEPTION_FLT_DIVIDE_BY_ZERO:       excName = "FLT_DIVIDE_BY_ZERO"; break;
        ...
        case EXCEPTION_STACK_OVERFLOW:           excName = "STACK_OVERFLOW (栈溢出)"; break;
    }
...
static void qtMessageHandler(QtMsgType type, const QMessageLogContext &ctx, const QString &msg)
{
    ...
    QByteArray utf8 = formatted.toUtf8();
    {
        ...
        if (copyLen >= RING_LINE_MAX) copyLen = RING_LINE_MAX - 1;
...
static void pruneOldLogs()
{
    ...
    qDebug().noquote() << "日志清理: 保留了" << MAX_LOG_FILES
                        << "份文件，删除了" << (files.size() - MAX_LOG_FILES) << "份旧文件";
}
...
void CrashHandler::install()
{
    ...
    QSysInfo sysInfo;
    snprintf(s_sysInfo, sizeof(s_sysInfo),
             "应用版本: %s\n"
             "操作系统: %s\n"
             "系统版本: %s\n"
             "CPU 架构: %s\n"
             ...
             "日志目录: %s\n",
             APP_VERSION,
             sysInfo.prettyProductName().toUtf8().constData(),
             sysInfo.productVersion().toUtf8().constData(),
             sysInfo.currentCpuArchitecture().toUtf8().constData(),
             QT_VERSION_STR,
             s_logDir);

    // 4. 本次启动创建新的会话日志文件：session_YYYYMMDD_HHMMSS.log
    QDateTime now = QDateTime::currentDateTime();
    QString sessionName = QStringLiteral("session_%1.log")
                              .arg(now.toString(QStringLiteral("yyyyMMdd_HHmmss")));
    snprintf(s_sessionPath, sizeof(s_sessionPath), "%s/%s",
             s_logDir, sessionName.toUtf8().constData());
    s_logFile = fopen(s_sessionPath, "w");
    ...
    qDebug().noquote() << QString("Version: %1  OS: %2  Qt: %3")
                          .arg(APP_VERSION)
