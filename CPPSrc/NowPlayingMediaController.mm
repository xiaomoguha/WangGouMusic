...
static OSStatus AudioDeviceChangedCallback(
...
- (void)startAudioDeviceMonitor
{
    ...
    AudioObjectAddPropertyListener(kAudioObjectSystemObject, &addr, AudioDeviceChangedCallback, (__bridge void *)self);
}
...
- (void)stopAudioDeviceMonitor
{
    ...
    AudioObjectRemovePropertyListener(
        kAudioObjectSystemObject, &addr, AudioDeviceChangedCallback, (__bridge void *)self
    );
}
    ...
    else
    {
        ...
        if (appIcon)
        {
            MPMediaItemArtwork *artwork =
                [[MPMediaItemArtwork alloc] initWithBoundsSize:appIcon.size
    ...
    NSURLSessionDataTask *task = [[NSURLSession sharedSession]
        dataTaskWithRequest:request
          completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
            ...
            {
                qWarning(
                    "NowPlaying: 封面下载失败(status=%ld err=%s)，使用 App 图标兜底", (long)(httpResp.statusCode),
...
- (void)applyAppIconArtwork
{
    ...
    if (!appIcon || appIcon.size.width == 0)
    {
        ...
        {
            appIcon =
                [[NSImage imageNamed:iconName] copy]
                    ?: [[NSImage alloc] initWithContentsOfFile:[[NSBundle mainBundle] pathForImageResource:iconName]];
        }
    }
    if (!appIcon)
    {
    ...
    MPMediaItemArtwork *artwork  = [[MPMediaItemArtwork alloc] initWithBoundsSize:appIcon.size
                                                                  requestHandler:^NSImage *(CGSize requestedSize) {
                                                                    return appIcon;
                                                                  }];
    self.cachedArtwork           = artwork;
