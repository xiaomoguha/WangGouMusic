...
AppUpdater::AppUpdater(QObject *parent)
    : QObject(parent), m_downloadManager(new QNetworkAccessManager(this)),
      m_currentVersion(QStringLiteral(APP_VERSION)), m_checkUrl(CHECK_UPDATE_URL)
{
}
...
QString AppUpdater::latestVersion() const
{
    return m_latestVersion;
}
QString AppUpdater::releaseNotes() const
{
...
double AppUpdater::downloadProgress() const
{
    return m_downloadProgress;
}
bool AppUpdater::downloading() const
{
...
void AppUpdater::checkForUpdate()
{
    ...
    ApiClient::instance().getJson(
        ...
        [this](QString err, int) { emit checkFailed(err); }, 10000
    );
}
...
void AppUpdater::downloadUpdate()
{
    ...
    QDir dir(tempDir);
    m_downloadedFilePath = dir.filePath(QStringLiteral("WangGouMusic_%1_setup.exe").arg(m_latestVersion));

    m_downloadFile = new QFile(m_downloadedFilePath, this);
    if (!m_downloadFile->open(QIODevice::WriteOnly))
    {
        emit downloadFailed(QStringLiteral("Cannot write to temp file: %1").arg(m_downloadFile->errorString()));
    ...
    request.setHeader(QNetworkRequest::UserAgentHeader, QStringLiteral("WangGouMusic/%1").arg(m_currentVersion));

    m_downloadReply = m_downloadManager->get(request);
    connect(m_downloadReply, &QNetworkReply::downloadProgress, this, &AppUpdater::onDownloadProgress);
    connect(m_downloadReply, &QNetworkReply::finished, this, &AppUpdater::onDownloadFinished);

    // 边下载边写入
    connect(
        m_downloadReply, &QIODevice::readyRead, this,
        [this]()
        {
